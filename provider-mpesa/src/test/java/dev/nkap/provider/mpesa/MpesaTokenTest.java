...
/**
 * {@link MpesaTokenCache.Token}'s {@code toString()}: the bearer token is a live credential until
 * it expires, so it is masked like the profile's.
 */
class MpesaTokenTest {

    // --- Token ---------------------------------------------------------------------------

    private static final Map<String, String> TOKEN_SECRETS = Map.of(
            "value", "canary-bearer-mpesa-5d2e");

    private static MpesaTokenCache.Token token() {
        Map<String, String> s = TOKEN_SECRETS;
        return new MpesaTokenCache.Token(s.get("value"), Instant.parse("2026-09-25T14:00:00Z"));
    }
    ...
    void every_token_component_is_printed_or_masked() {
        RecordToString.assertEveryComponentPrintedOrMasked(token(), Set.of("expiresAt"),
                TOKEN_SECRETS.keySet(), MpesaTokenCache.Token.MASKED);
    }
}
