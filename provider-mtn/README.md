...
| `MtnRequestRejected` | Unchecked. MTN answered a submission with `400` — a definitive "this request is invalid", which `ProviderAdapter.submit` has no declared way to report. |

## How answers are mapped

**submit** — `202` is acknowledged (`SUBMITTED`); the body is empty by contract. A `409` is
*also* acknowledged: the reference was already used, which — since Nkap persists it before
calling — means a previous attempt reached MTN, and the query will settle it. A `400` is
`MtnRequestRejected`. Anything else, a timeout, or an unreadable body is
...
comes out `UNKNOWN`. A `404` (`RESOURCE_NOT_FOUND`) is **`UNKNOWN`, not a failure** — Nkap
persists the reference first, so a 404 is "never arrived" or "not visible yet", and one
response cannot tell them apart; the reconciler concludes after its window. Anything else is
`ProviderUnavailableException`.
