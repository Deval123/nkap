...
- **A deployment whose operator credential starts or ends with whitespace stops starting after
  this upgrade.** Such a deployment was already broken: the credential was sent as configured,
  and the operator refused every request. Nothing in any log said why, because whitespace is
  invisible and the value is never printed. Now `MtnProfile` and `MpesaProfile` refuse it at
  startup, naming the field and which end, never the value, whether it came from a variable or
  a file. A no-break space pasted from a web portal counts. Remove the whitespace; the gateway
  does not strip it. A padded MTN country (`" cm"`) keeps working as before: the profile now
