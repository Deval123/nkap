...
class MigrationRollbackIT {
    ...
    @Test
    @DisplayName("the inverse of V6 drops exactly the api_key table, leaving the V5 schema intact")
    void the_inverse_of_v6_returns_the_schema_to_v5() throws IOException {
        try (PostgreSQLContainer<?> postgres = new PostgreSQLContainer<>(PostgresDatabase.IMAGE)) {
            postgres.start();
            JdbcTemplate jdbc = jdbcFor(postgres);

            migrateTo(postgres, "5");
            Set<String> afterV5 = schemaObjects(jdbc);
            assertThat(userTables(jdbc)).doesNotContain("api_key");
    ...
    @Test
    @DisplayName("the inverse of V8 removes exactly the refund columns and index, leaving the V7 schema intact")
    void the_inverse_of_v8_returns_the_schema_to_v7() throws IOException {
        try (PostgreSQLContainer<?> postgres = new PostgreSQLContainer<>(PostgresDatabase.IMAGE)) {
            postgres.start();
            JdbcTemplate jdbc = jdbcFor(postgres);

            migrateTo(postgres, "7");
            Set<String> afterV7 = schemaObjects(jdbc);
            Set<String> paymentColumnsAfterV7 = columnsOf(jdbc, "payment");
            assertThat(paymentColumnsAfterV7).doesNotContain("refund_of", "refunded_minor");
            ...
            assertThat(columnsOf(jdbc, "payment")).isEqualTo(paymentColumnsAfterV7);
            assertThat(indexesOf(jdbc, "payment")).doesNotContain("payment_refund_of_idx");

            // V8 forward again — the inverse removed its history row.
            migrate(postgres);
            assertThat(columnsOf(jdbc, "payment")).contains("refund_of");
        }
    }
    ...
    @Test
    @DisplayName("the inverse of V5 drops exactly the statement tables and triggers, leaving the V4 schema intact")
    void the_inverse_of_v5_returns_the_schema_to_v4() throws IOException {
        try (PostgreSQLContainer<?> postgres = new PostgreSQLContainer<>(PostgresDatabase.IMAGE)) {
            postgres.start();
            JdbcTemplate jdbc = jdbcFor(postgres);

            migrateTo(postgres, "4");
            Set<String> afterV4 = schemaObjects(jdbc);
            assertThat(userTables(jdbc)).doesNotContain("statement_import", "statement_line", "statement_finding");
            ...
            assertThat(schemaObjects(jdbc)).isEqualTo(afterV4);

            // V5 forward again — the inverse removed its history row.
            migrate(postgres);
            assertThat(userTables(jdbc)).contains("statement_line");
        }
    }
    ...
    @Test
    @DisplayName("the inverse of V10 drops exactly the revoked_at column, leaving the V9 schema intact")
    void the_inverse_of_v10_returns_the_schema_to_v9() throws IOException {
        try (PostgreSQLContainer<?> postgres = new PostgreSQLContainer<>(PostgresDatabase.IMAGE)) {
            postgres.start();
            JdbcTemplate jdbc = jdbcFor(postgres);

            migrateTo(postgres, "9");
            Set<String> afterV9 = schemaObjects(jdbc);
            Set<String> apiKeyColumnsAfterV9 = columnsOf(jdbc, "api_key");
            assertThat(apiKeyColumnsAfterV9).doesNotContain("revoked_at");
    ...
    @Test
    @DisplayName("the inverse of V11 drops exactly the provider_reference index, leaving the V10 schema intact")
    void the_inverse_of_v11_returns_the_schema_to_v10() throws IOException {
        try (PostgreSQLContainer<?> postgres = new PostgreSQLContainer<>(PostgresDatabase.IMAGE)) {
            postgres.start();
            JdbcTemplate jdbc = jdbcFor(postgres);

            migrateTo(postgres, "10");
            Set<String> afterV10 = schemaObjects(jdbc);
